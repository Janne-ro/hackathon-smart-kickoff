...
This guide captures the full machine setup we completed for a video AI agent with avatar, voice, LLM, Thymia biomarker integration, and optional Shen camera vitals.
...
- `hackathon-smart-kickoff` (team repo and shared docs)
...
6. Optional Shen.AI browser SDK measures camera vitals

## 2) Repositories Cloned

Assume a workspace root called `ROOT`.

Example:

```bash
export ROOT=~/Desktop/repos/preply-hackathon
```

Repositories cloned under `ROOT`:

- `https://github.com/AgoraIO-Conversational-AI/agent-samples`
- `https://github.com/AgoraIO-Conversational-AI/server-custom-llm`
- `https://github.com/Janne-ro/hackathon-smart-kickoff`
...
```bash
cd "$ROOT/agent-samples/simple-backend"
python3 -m venv venv
...
```bash
cd "$ROOT/agent-samples/react-video-client-avatar"
...
```bash
cd "$ROOT/server-custom-llm/node"
npm install --legacy-peer-deps
```
...
```bash
cd "$ROOT/server-custom-llm/go-audio-subscriber/sdk"
bash scripts/install_agora_sdk.sh

cd "$ROOT/server-custom-llm/go-audio-subscriber"
make build
```

Binary created:

- `"$ROOT/server-custom-llm/go-audio-subscriber/bin/audio_subscriber"`

## 5) Coding Assistant Skill Setup

Agora skill was linked into Codex skills:

- Source: `"$ROOT/skills/skills/agora"`
- Linked to: `~/.codex/skills/agora`

Command used:

```bash
ln -sfn "$ROOT/skills/skills/agora" ~/.codex/skills/agora
```
...
- `"$ROOT/agent-samples/simple-backend/.env"`

This file should include these profiles:

- `VIDEO_*` for avatar video client mode
- `THYMIA_*` for custom LLM + biomarker mode
- `VIDEO_THYMIA_SHEN_*` for combined avatar + Thymia + Shen mode
...
### Important combined profile setting

For Shen + Thymia together:

```bash
VIDEO_THYMIA_SHEN_LLM_URL=http://localhost:8100/chat/completions
```

For actual cloud-reachable agent traffic, this also must be public.

## 6.2 React client feature flags

File created:

- `"$ROOT/agent-samples/react-video-client-avatar/.env.local"`
...
Without `NEXT_PUBLIC_SHEN_API_KEY`, the Shen tab may render but real vitals measurement will not start.
...
```bash
cd "$ROOT/server-custom-llm/node"
PORT=8100 THYMIA_ENABLED=true SHEN_ENABLED=true node custom_llm.js
```
...
### Start agent (VIDEO_THYMIA_SHEN)

```bash
curl -s -i "http://127.0.0.1:8082/start-agent?channel=video_thymia_shen_check_01&profile=VIDEO_THYMIA_SHEN"
```
...
- Shen data not appearing:
  - Confirm `NEXT_PUBLIC_ENABLE_SHEN=true` in client `.env.local`.
  - Confirm `NEXT_PUBLIC_SHEN_API_KEY` is set.
  - Confirm browser camera permission was granted.
  - Confirm custom LLM is started with `SHEN_ENABLED=true`.

- Custom LLM unreachable from Agora cloud:
  - `THYMIA_LLM_URL` must be public for real cloud calls.
  - `VIDEO_THYMIA_SHEN_LLM_URL` must also be public for combined mode.
  - In some corporate networks, Cloudflare Quick Tunnels may be blocked. If so, use an allowed public endpoint or deployed server instead.
...
For the combined wellness demo, prefer `VIDEO_THYMIA_SHEN`.
...
```text
Clone these repos into a local workspace root, for example:

ROOT=~/Desktop/repos/preply-hackathon

Clone these repos under that root:
- AgoraIO-Conversational-AI/agent-samples
- AgoraIO-Conversational-AI/server-custom-llm
- Janne-ro/hackathon-smart-kickoff
- thymia-ai/preply-thymia-hackathon
- agoraio/skills

Read AGENT.md in agent-samples first.
Set up VIDEO, THYMIA, and VIDEO_THYMIA_SHEN profiles in simple-backend/.env using profile-prefixed keys.
...
Set NEXT_PUBLIC_ENABLE_THYMIA=true and NEXT_PUBLIC_ENABLE_SHEN=true in react-video-client-avatar/.env.local.
Set NEXT_PUBLIC_SHEN_API_KEY if available.
Start backend on :8082, client on :8084, custom LLM on :8100 with THYMIA_ENABLED=true SHEN_ENABLED=true.
Run curl health checks and start-agent checks for VIDEO, THYMIA, and VIDEO_THYMIA_SHEN.
If the custom LLM must be reachable from Agora cloud, expose it with a public URL and update THYMIA_LLM_URL and VIDEO_THYMIA_SHEN_LLM_URL.
